...
public class ValidCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (game == null) {
            // Fallback: if there is exactly one game, use it
        ...
        // Determine objective index in the current grid for advancements
        ...
        final int objIndex = java.util.stream.IntStream.range(0, grid.size())
                ...
                .findFirst().orElse(-1);
        plugin.getObjectiveManager().validateObjective(fGame.getId(), teamId, fObjective.getId()).thenAccept(success -> {
            Bukkit.getScheduler().runTask(plugin, () -> {
                if (success) {
                    ...
                    // Grant the advancement for this objective to members of the validated team
                    if (objIndex >= 0) {
                        for (java.util.UUID u : fTeam.getPlayers()) {
                            Player tp = Bukkit.getPlayer(u);
                            if (tp != null) plugin.getAdvancementManager().grantObjectiveAdvancement(fGame.getId(), objIndex, tp);
                        }
                    }
