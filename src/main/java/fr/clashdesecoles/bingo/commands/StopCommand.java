...
public class StopCommand implements CommandExecutor {
    ...
    public StopCommand(BingoPlugin plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (gameId == null) {
            sender.sendMessage("§cUsage: /bingo stop [idPartie]");
            return true;
        }
        
        if (plugin.getGameManager().stopGame(gameId)) {
            plugin.getTimerManager().stop();
            Bukkit.broadcastMessage(MessageUtil.getRaw("timer.stop-broadcast"));
        } else {
            sender.sendMessage(MessageUtil.get("timer.stop-error"));
        }
